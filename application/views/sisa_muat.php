...
<title>Dashboard Rekom antrian</title>
	<style>
		.blink {
...
<div class="container" >
	...
	<form action="<?php echo $form?>" method="post" class="mt-3">
		<input type="date" name="tgl_awal" value="<?php echo  isset($tgl_awal) ? $tgl_awal : ''; ?>" />
		<input type="date" name="tgl_akhir" value="<?php echo  isset($tgl_akhir) ? $tgl_akhir : ''; ?>" />
		<input type="submit" class="btn btn-success" value="Submit" onclick="javascript: form.action='<?php echo $form?>';"/>
	</form>
	<!-- rumah -->
		<?php 
			foreach($gudang as $gd):
			$locid= substr($gd->locid,0,4);
		?>
		<div class="col-md-4 mt-3" >
			<div class="server-rack" >			
				<p class="label" id="label"><?php echo $locid;?></P>
				<?php foreach($listed as $ls) :
					if($ls->locid==$locid):
					?>
				<div class="server">
				  <p class="server-info" id="call">
					<span><?php echo $ls->nodo.'-'.$ls->nopol?></span>
				  </p>
				</div>
				<?php 
				endif;
